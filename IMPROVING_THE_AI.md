...
| P0 Measurement | **shipped + verified — gate accruing** | Implemented: durable append-only `realized_trades` journal (v15, UNIQUE per position, written at close, NEVER touched by reset) with verdict/provenance-flags/exit-reason/dd@5m per row; `paper_trades` fills now carry position_id+flags; `/paper/reset` guarded ({confirm:true} required, HTTP 400 otherwise) + auto-export + `paper_resets` audit row; `/paper` exposes ledger + reconcile delta; `/paper/equity` realized curve; UI: ledger line + equity canvas + DD@5M/EXIT-REASON columns. VERIFIED LIVE 2026-06-12: operator's real reset exercised the full guard (2.4MB export, 440 open/989 closed/2,568 fills preserved, journal's 27 rows survived); first fresh-generation close ($BOOGLEMAN, stop −47%) reconciled EXACTLY — cash-derived realized = Σ fills = journal Σ (delta 0.000); Playwright screenshot evidence, zero console errors; 402/402 tests. **Gate: panels reconcile ✓ TODAY; "7 unbroken days of ledger" accrues from 2026-06-12 14:05 UTC → completes 2026-06-19.** Re-run `node scripts/research/_probe_p0_reconcile.mjs` to re-check any time. | 2026-06-12 |
